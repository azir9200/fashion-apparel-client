...
const BrandShow = ({ brand }) => {
  const { _id, name, photo, brandName, price, description, rating } = brand;
    ...
    <div>
      <div className=" flex flex-col  text-gray-700 bg-white shadow-md rounded-xl bg-clip-border">
        ...
        <div className="p-6 pt-0 gap-4">
          ...
          <Link to={`/updateCartDetails/${_id}`}>
            <button
              className="block w-full bg-green-300  select-none rounded-lg bg-blue-gray-900/10 py-3 px-6 text-center align-middle font-sans text-xs font-bold uppercase text-blue-gray-900 transition-all hover:scale-105 focus:scale-105 focus:opacity-[0.85] active:scale-100 active:opacity-[0.85] disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none m-2"
              type="button"
            >
